...
impl State {
    pub fn get_api_key(&self, provider: ProviderId) -> Option<ApiKey> {
        self.api_keys.get(&provider).cloned()
    }

    pub fn insert_api_key(&mut self, provider: ProviderId, api_key: ApiKey) {
        self.api_keys.insert(provider, api_key);
    }

    pub fn remove_api_key(&mut self, provider: &ProviderId) {
